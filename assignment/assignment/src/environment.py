...
##Open the config file to read configurable data
    ...
    initialise_logger(context, feature)
    if 'ui_cloudmore_test' in feature.tags:
        ###read the cloudmore link from config file and also read the chrome exe file location for browser launch
        cloudmore_url = config['configuration_setting']['cloudmoreurl']
        chrome_exe_location = config['configuration_setting']['chrome_exe_location_path']

        context.catalogue_driver = ui_utility.initialise_webdriver(context,cloudmore_url, chrome_exe_location)
